...
| `dist-white-label/` | Unbranded build output — the same app carrying no name. Generated by `npm run build:white-label`, which `build:all` runs, and CI rejects a stale copy just as it does the branded one. | No — it's regenerated. |
...
└── assets/
    ...
    ├── js/
    │   ├── 00-brand.ts             The app's name, from .env — one place for a rebrand
...
npm run build:white-label  # -> dist-white-label/: the same app, carrying no name
...
### Renaming the app, or shipping it unbranded

The name lives in **`.env`** and nowhere else. `assets/js/00-brand.ts` reads it
and every module and `index.html` go through that, so a rebrand is four values
in one file:

```bash
VITE_BRAND_NAME=Ariadne Maps      # tab title, Learn heading, welcome dialog, export stamps
VITE_BRAND_SHORT=Ariadne          # mid-sentence in the lessons; blank reads as "This app"
VITE_BRAND_TAGLINE=Map cause and effect
VITE_BRAND_MARK=on                # "off" removes the mark, wordmark and favicon
```

`npm run build:white-label` loads **`.env.whitelabel`** — every value blank —
and writes `dist-white-label/systems-map.html`. That build carries no name at
all: not hidden behind a CSS rule, **absent**. Three of the brand elements are
shapes rather than text (the favicon, the header mark, the empty state's
wordmark), so they are wrapped in `<!--brand--> … <!--/brand-->` in
`index.html` and a Vite plugin drops the markup outright. Both builds end with
`scripts/check-brand.mjs`, which fails the build if an unbranded artifact still
contains the name — or if a normal one does not.

Anything the app **writes** stamps the name too: the header comment on an
exported spreadsheet and the title of a published view-only page. That is
provenance, not decoration — it is how somebody opening the file in a year can
tell what produced it — so for a client build, fill the values in rather than
emptying them.

The example maps in `assets/data/` name no product in their header comments, so
they carry nothing into a build or into anything exported from them.
