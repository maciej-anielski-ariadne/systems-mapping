...
| `tools/` | Standalone analysis tools, not part of the app — open them directly and drop your own CSV in. [`pathway-atlas.html`](tools/PATHWAY-ATLAS.md) shows **everything downstream of one box** as a single expandable structure, without enumerating a single route. [`strand-condenser.html`](tools/README.md) is the earlier tool, which walks every route between two boxes instead. | Only if a strand has more routes than you can read. |
...
[`tools/pathway-atlas.html`](tools/PATHWAY-ATLAS.md) takes one box and shows **everything downstream of it** as a single expandable structure — a shared spine, and at each split a set of alternatives carrying an exact count and a badge naming which lanes take it. It never enumerates a route. The complete set of pathways is already the subgraph the box can reach, so the page condenses that subgraph instead: feedback loops contract to one element, boxes whose names differ by a lane value (`Cat A Targets` / `Cat B Targets`) group into one role, and the totals come from arithmetic rather than counting. A 296-box map builds in about 15ms with no sampling, budget or cap.
