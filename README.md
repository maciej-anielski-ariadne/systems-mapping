...
- **Zoom** → bottom-right `−` / `+` buttons, `Ctrl/Cmd` + scroll-wheel (or trackpad pinch), or `Ctrl/Cmd + =/-/0` to zoom in / out / reset. Pinching anchors on the cursor. Clicking the percentage **fits the whole map in the frame** — which is also what happens when a map is loaded, so you never start cropped. Fitting only ever zooms out: a map smaller than the frame keeps its size and is centred instead.
