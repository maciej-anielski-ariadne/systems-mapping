...
| `tools/` | Standalone analysis tools, not part of the app. [`strand-condenser.html`](tools/README.md) walks every route between two boxes, folds away the repeated lanes (`Cannabis Seizure` / `Cocaine Seizure` / …) and shows only where the pathways actually differ. Open it directly and drop your own CSV in. | Only if a strand has more routes than you can read. |
...
On a large map a single pair of boxes can have hundreds of thousands of routes between them, and the strongest ten can all be small variations on one chain. [`tools/strand-condenser.html`](tools/README.md) is a standalone page for condensing that list on your own map: it walks every route, folds together the ones that differ only in which parallel lane they ran through (`Cannabis Seizure` / `Cocaine Seizure` / …), and surfaces the sections where the lanes genuinely diverge.
