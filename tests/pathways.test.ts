...
import { state, edgeById, nodeById } from "../assets/js/03-state";
...
// A strand through a box the category filters have taken off the map: pathway
// mode reopens just enough of the sidebar to draw it. "Just enough" is per tag
// CLASS, because that is how isNodeVisible judges a box — reopening a corner
// tag does nothing for a box that went dark on its fill tags.
const TAGGED_CSV = `# SECTION: streams
id,label,short,color
one,One,ONE,#60a5fa
...
s1c,Corner One,#f59e0b,#111111,secondary
s2c,Corner Two,#ef4444,#111111,secondary

# SECTION: nodes
id,label,description,stream,stage,category,baseline,unit,controllable,direction,slider_max
src,Source,,one,s1,p1,,,,,
mid,Middle,,one,s2,p2|s1c|s2c,,,,,
dst,Dest,,one,s3,p1,,,,,

# SECTION: edges
from,to,effect,elasticity,style,description
src,mid,increases,0.5,,
mid,dst,increases,0.5,,
`;

describe("pathway mode reopens category filters per tag class", () => {
  beforeEach(() => loadDataFromCsv(TAGGED_CSV));

  it("reopens the fill tag when the strand's box went dark on its fills", () => {
    state.hiddenCategories = new Set(["p2"]);
    expect(isNodeVisible(nodeById.mid)).toBe(false);

    startPathway("src", "dst");
    expect(pathwayActive()).toBe(true);
    expect(pathwayReopenedFilters()).toBe(true);
    expect(state.hiddenCategories.has("p2")).toBe(false);
    expect(isNodeVisible(nodeById.mid)).toBe(true);

    clearPathway();
    expect(state.hiddenCategories.has("p2")).toBe(true); // put back as the user left it
  });

  it("reopens a corner tag when the strand's box went dark on its corners", () => {
    // The box still has its fill tag, so a rule that only reopened when EVERY
    // tag was hidden would leave it off the map and the strand undrawable.
    state.hiddenCategories = new Set(["s1c", "s2c"]);
    expect(isNodeVisible(nodeById.mid)).toBe(false);

    startPathway("src", "dst");
    expect(isNodeVisible(nodeById.mid)).toBe(true);
    expect(state.hiddenCategories.has("s1c")).toBe(false);

    clearPathway();
    expect(state.hiddenCategories.has("s1c")).toBe(true);
  });

  it("reopens one tag of each dark class at once", () => {
    state.hiddenCategories = new Set(["p2", "s1c", "s2c"]);
    startPathway("src", "dst");
    expect(state.hiddenCategories.has("p2")).toBe(false);
    expect(state.hiddenCategories.has("s1c")).toBe(false);
    expect(isNodeVisible(nodeById.mid)).toBe(true);
  ...
  it("leaves the filters alone when every box on the strand is still drawable", () => {
    state.hiddenCategories = new Set(["s1c"]); // mid keeps corner s2c
    startPathway("src", "dst");
    expect(pathwayActive()).toBe(true);
    expect(pathwayReopenedFilters()).toBe(false);
    expect(state.hiddenCategories.has("s1c")).toBe(true);
  });
});
