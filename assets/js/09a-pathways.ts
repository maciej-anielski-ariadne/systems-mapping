...
function openFiltersForRoute(route: PathwayRoute | null): void {
  ...
  for (const id of route.nodeIds) {
    ...
    // A category filter removes a box when a whole CLASS of its tags is hidden
    // (all its fill tags, or all its corner tags — isNodeVisible judges the two
    // separately), so reopen one tag of each dark class — not every tag.
    const byClass = splitCategoriesByClass(nodeCategoryIds(node));
    for (const tags of [byClass.primary, byClass.secondary]) {
      if (tags.length && tags.every(c => state.hiddenCategories.has(c))) {
        if (!categories.includes(tags[0])) categories.push(tags[0]);
      }
    }
  }
