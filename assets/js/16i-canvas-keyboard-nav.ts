...
import { computeLayout, slotTopY } from "./08-layout";
import { render } from "./11-rendering";
import { createNodeInCell, commitNewEdge } from "./16e-canvas-edit";
import { inlineRenameAppend, commitInlineRename } from "./16h-canvas-inline-rename";
...
export function moveCursorToSlot(streamId: string, stageId: string, slotIndex: number): void {
...
export function moveCursorToCell(streamId: string, stageId: string): void {
...
export function scrollCellIntoView(streamId: string, stageId: string, slotIndex: number): void {
...
export function streamIndexFor(streamId: string): number { return STREAMS.findIndex(s => s.id === streamId); }
export function stageIndexFor (stageId: string): number  { return STAGES.findIndex (s => s.id === stageId);  }
...
export function nextVisibleStageIndex(fromIdx: number, step: number): number {
...
export function moveCanvasCursor(dStream: number, dStage: number): boolean {
...
export function createNodeAtCursorWithChar(firstChar: string): boolean {
  const pos = getCanvasCursorPosition();
...
export function handleCanvasTab(direction: string): boolean {
  const pos = getCanvasCursorPosition();
  if (!pos) return false;
  const cIdx = stageIndexFor(pos.stageId);
...
export function handleCanvasEnterCreate(): boolean {
  const pos = getCanvasCursorPosition();
  if (!pos) return false;
  if (typeof commitInlineRename === "function") commitInlineRename();
  if (state.canvasEdit) state.canvasEdit.cursorCell = null;
...
export function openCanvasEdgePicker(fromNodeId: string): void {
  ...
  const nativeSelect = overlay.querySelector("select") as HTMLSelectElement | null;
  const typableInput = overlay.querySelector(".typeable-dropdown-input") as HTMLInputElement | null;
...
export function positionEdgePicker(overlay: HTMLElement, fromNodeId: string): void {
...
export function closeCanvasEdgePicker(): void {
...
export function cssEscapeForCanvas(s: string): string {
