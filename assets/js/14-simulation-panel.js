...
function renderSimulationPanel() {
  ...
  simPanel.querySelectorAll(".sim-slider").forEach(slider => {
    slider.addEventListener("input", event => {
      ...
      applySimMultiplier(nodeId, newMultiplier, event.target);
...
function applySimMultiplier(nodeId, newMultiplier, originElement) {
...
function syncSimRow(nodeId, originElement) {
  ...
  if (slider && slider !== originElement) slider.value = multiplier;

  const valueInput = row.querySelector(".sim-value-input");
...
function maybeRefreshDetailPanel(changedNodeId) {
...
function updateDetailPanelDeltaInline(changedNodeId) {
  ...
  if (rows[2]) {
    const valueCell = rows[2].querySelector(".detail-quant-value");
    if (valueCell) {
  ...
  if (selectedId !== changedNodeId && rows[1]) {
    const cell = rows[1].querySelector(".detail-quant-value");
