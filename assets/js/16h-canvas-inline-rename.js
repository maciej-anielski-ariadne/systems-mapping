...
function inlineRenameAppend(char) {
...
// the node.
function inlineRenameBackspace() {
  if (!state.canvasEdit || !state.canvasEdit.inlineRename) return;
  const ir = state.canvasEdit.inlineRename;
  const node = nodeById[ir.nodeId];
  if (!node) {
...
function commitInlineRename(options) {
...
function initCanvasInlineRename() {
