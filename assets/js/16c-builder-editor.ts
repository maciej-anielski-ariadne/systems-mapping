...
import { builderTabNavigate } from "./16d-builder-events";
...
interface CellEditorState {
  ...
  openSafetyTimeout: ReturnType<typeof setTimeout> | null;
  startWidth: number;
  startHeight: number;
}

interface HideCellEditorOptions {
...
export const CELL_EDITOR_TYPES_SELECTOR =
  '.builder-table input[type="text"]:not(.typeable-dropdown-input), ' +
  '.builder-table input[type="number"]';
export let cellEditorState: CellEditorState | null = null;
export let _dismissedTrigger: HTMLInputElement | null = null;   // remembered until focus leaves this element

export function cellOverflows(input: HTMLInputElement | null | undefined): boolean {
...
export function clearDismissedTrigger(): void { _dismissedTrigger = null; }

export const CELL_EDITOR_MAX_WIDTH  = 560;   // ~60 chars at mono 12px — comfortable read width
export const CELL_EDITOR_MAX_HEIGHT = 360;   // hard cap; user scrolls inside beyond this
export const CELL_EDITOR_SAFETY_MS  = 220;   // > CSS transition duration, for transitionend miss cleanup
...
export let _cellEditorMirror: HTMLDivElement | null = null;
export function getCellEditorMirror(): HTMLDivElement {
...
export function removeCellEditorMirror(): void {
...
// "edit text".
export function ariaLabelForTrigger(trigger: HTMLInputElement): string {
  ...
  const ths = table ? table.querySelectorAll("thead th") : ([] as unknown as NodeListOf<HTMLElement>);
...
// the size transition.
export function setCellEditorAnchor(editor: HTMLTextAreaElement, trigger: HTMLInputElement): void {
...
export function fitEditorToText(
  editor: HTMLTextAreaElement,
  trigger: HTMLInputElement,
  startWidth: number,
  startHeight: number,
...
export function openCellEditor(trigger: HTMLInputElement): void {
  ...
  void editor.offsetHeight; // force a synchronous reflow
  void backdrop.offsetHeight; // force a synchronous reflow
  ...
  editor.addEventListener("keydown", (event: KeyboardEvent) => {
    ...
    if (event.key === "Tab") {
      ...
      const result: { atStart?: boolean; atEnd?: boolean; moved?: HTMLElement; addedRow?: boolean } = (typeof builderTabNavigate === "function")
...
export function hideCellEditor(options?: HideCellEditorOptions): void {
...
export function handleBuilderFocus(event: FocusEvent): void {
  if (!(event.target as HTMLInputElement).matches(CELL_EDITOR_TYPES_SELECTOR)) return;
  ...
  if (cellOverflows(event.target as HTMLInputElement)) openCellEditor(event.target as HTMLInputElement);
}
...
// cell starts fresh.
export function handleBuilderFocusOut(event: FocusEvent): void {
...
export function handleBuilderInputForOverflow(event: Event): void {
